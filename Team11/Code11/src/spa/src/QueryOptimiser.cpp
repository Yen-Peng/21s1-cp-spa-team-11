...
bool QueryOptimiser::optimiseQuery() {
    ...
    bool manyAffects = 100 * affectsNum / PKB::getInstance().getTotalNumOfVars() > HIGH_AFFECTS_PERCENTAGE;
    bool manyVars = PKB::getInstance().getTotalNumOfVars() > PKB::getInstance().getTotalNumberOfStatements();
...
//        cout << QueryClause::isSameClause(a, prev) << endl;
        if (first || !QueryClause::isSameClause(a, prev)) {
            cout << "adding to unique clauses: " << endl;
            cout << a.to_string() << endl;
            cout << "end of adding" << endl;
            duplicatesRemoved.push_back(a);
        } else {
            cout << "SAME CLAUSE" << endl;
            cout << a.getReversePriority(false, false) << endl;
            cout << prev.getReversePriority(false, false) << endl;
        }
        first = false;
        prev = a;
    }
...
//    cout << "incrementing freqs" << endl;
//    cout << syn1 << " " << freq1 << endl;
//    cout << syn2 << " " << freq2 << endl;
    return true;
}
