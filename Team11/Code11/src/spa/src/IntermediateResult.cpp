...
struct compareRow {
    inline bool operator() (vector<QueryElement> row1, vector<QueryElement> row2)
    {
        ...
        while (index < size) {
            assert((*(row1It + index)).TYPE == (*(row2It + index)).TYPE);
...
//        cout << "adding to intermediate results...\n" << doubleColumnCPS.to_string();
...
bool IntermediateResult::getResultBoolean() {
    cout << "getResultBoolean:" << endl;
    reduceAllResults();
    bool hasResult = true;
    for (auto& a : singleColumnResults) {
        cout << a.first << a.second.empty() << endl;
        hasResult &= !a.second.empty();
        if (!hasResult) {
            return false;
        }
...
bool IntermediateResult::areEqualRows(vector<QueryElement> row1, vector<QueryElement> row2) {
    ...
    while (index < size) {
        ...
        if ((*row1It).TYPE != (*row2It).TYPE) {
            cout << "debug" << (*row1It).to_string() << " " << (*row2It).to_string() << endl;
            cout << SetProcessor::to_string(row1) << endl;
            cout << SetProcessor::to_string(row2) << endl;
            cout << ::to_string(index+1) + "/" + ::to_string(size);
            return false;
        }
...
//        unordered_set<string>::iterator cit = c.begin();
    ...
    for (string s1 : cluster) {
        ...
        for (string s2 : cluster) {
            ...
            cout << SetProcessor::to_string(SetProcessor::set_to_vector<string>(erased)) << endl;
    ...
    *res = SetProcessor::crossProductPtr<QueryElement>(tablesToCross)->limitToColumns(header);
    cout << "printing cps: " << res->to_string();
    sort(res->body.begin(), res->body.end(), compareRow());
    res->body.erase(unique(res->body.begin(), res->body.end(), areEqualRows), res->body.end());
    return res;
}
